(function($) {
      ...
      $(window).resize(function() {
         ...
         $('#inner-container, #inner-container > *').height(winHeight - 42);
      }).resize();
      ...
      $('#btnRun').click(function(){
         ...
         var runText = $(this).val();

         if (! data) return false;

         $(this).val('Executing...').css({opacity:'.5', cursor: 'wait'}).attr('id', 'noRun');
         $.post('includes/gate.php', {"data": data}, function(res){
            ...
            $('#noRun').val(runText).css({opacity:'1', cursor: 'pointer'}).attr('id', 'btnRun');
         });
      });
   ...
   $('#btnSave').click(function(){
      ...
      for (var i = 0, l = localStorage.length; i < l; i++){
         if (localStorage.key(i).indexOf('pe_snippet') > -1)
         {
            ...
            $('#snippets').append('<li rel="' + key + '"><a class="left" title="Delete" href="#">&times;</a><div class="left">&nbsp;</div><a href="#">' + key.replace('pe_snippet', '') + '</a></li>');
         }
      }
   ...
   $('#snippets').on('click', 'li a', function(){
